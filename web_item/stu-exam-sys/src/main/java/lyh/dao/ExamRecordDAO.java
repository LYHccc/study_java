package lyh.dao;

import lyh.model.*;
import lyh.util.DBUtil;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.util.ArrayList;
import java.util.Date;
import java.util.List;

public class ExamRecordDAO {

    public static List<ExamRecord> query(Page p) {
        List<ExamRecord> records = new ArrayList<>();
        Connection connection = null;
        PreparedStatement statement = null;
        ResultSet resultSet = null;

        try{
            connection = DBUtil.getConnection();
            StringBuilder sql = new StringBuilder("select" +
                    "  er.id," +
                    "  er.score," +
                    "  er.exam_record_desc," +
                    "  er.create_time," +
                    "  e.id            prefix_e_id," +
                    "  e.exam_name     prefix_e_exam_name," +
                    "  e.exam_desc     prefix_e_exam_desc," +
                    "  c.id            prefix_c_id," +
                    "  c.classes_name  prefix_c_classes_name," +
                    "  c2.id           prefix_c2_id," +
                    "  c2.course_name  prefix_c2_course_name," +
                    "  s.id            prefix_s_id," +
                    "  s.create_time   prefix_s_create_time," +
                    "  s.student_name  prefix_s_student_name," +
                    "  s.student_no    prefix_s_student_no," +
                    "  s.id_card       prefix_s_id_card," +
                    "  s.student_email prefix_s_student_email" +
                    " from exam_record er" +
                    "  join exam e on er.exam_id = e.id" +
                    "  join classes c on e.classes_id = c.id" +
                    "  join course c2 on e.course_id = c2.id" +
                    "  join student s on c.id = s.classes_id");
            if(p.getSearchText() != null){
                sql.append(" where s.student_name like ?");
            }

            //查询总条数
            StringBuilder countSql = new StringBuilder("select count(0) count from(");
            countSql.append(sql);
            countSql.append(")tmp");
            statement = connection.prepareStatement(countSql.toString());
            if(p.getSearchText() != null){
                statement.setString(1, "%" + p.getSearchText() + "%");
            }
            resultSet = statement.executeQuery();
            while(resultSet.next()){
                int total = resultSet.getInt("count");
                Response.setPageTotal(total);
            }

            //查询表格数据
            sql.append(" order by er.create_time " + p.getSortOrder());
            sql.append(" limit ?, ?");
            statement = connection.prepareStatement(sql.toString());
            int i = 1;
            if(p.getSearchText() != null){
                statement.setString(i, "%" + p.getSearchText() + "%");
                i++;
            }
            statement.setInt(i, (p.getPageNumber() - 1) * p.getPageSize() + 1);
            i++;
            statement.setInt(i, p.getPageSize());

            resultSet = statement.executeQuery();
            while(resultSet.next()){
                //设置考试成绩对象
                ExamRecord er = new ExamRecord();
                er.setId(resultSet.getInt("id"));
                er.setScore(resultSet.getBigDecimal("score"));
                er.setExamRecordDesc(resultSet.getString("exam_record_desc"));
                er.setCreateTime(new Date(resultSet.getTimestamp("create_time").getTime()));

                //设置考试对象
                Exam exam = new Exam();
                exam.setId(resultSet.getInt("prefix_e_id"));
                exam.setExamName(resultSet.getString("prefix_e_exam_name"));
                exam.setExamDesc(resultSet.getString("prefix_e_exam_desc"));
                er.setExam(exam);

                //设置班级对象
                Classes classes = new Classes();
                classes.setId(resultSet.getInt("prefix_c_id"));
                classes.setClassesName(resultSet.getString("prefix_c_classes_name"));
                er.setClasses(classes);

                //设置课程对象
                Course course = new Course();
                course.setId(resultSet.getInt("prefix_c2_id"));
                course.setCourseName(resultSet.getString("prefix_c2_course_name"));
                er.setCourse(course);

                //设置学生对象
                Student student = new Student();
                student.setId(resultSet.getInt("prefix_s_id"));
                student.setCreateTime(new Date(resultSet.getTimestamp("prefix_s_create_time").getTime()));
                student.setStudentName(resultSet.getString("prefix_s_student_name"));
                student.setStudentNo(resultSet.getString("prefix_s_student_no"));
                student.setIdCard(resultSet.getString("prefix_s_id_card"));
                student.setStudentEmail(resultSet.getString("prefix_s_student_email"));
                er.setStudent(student);

                records.add(er);
            }
        } catch (Exception e) {
            throw new RuntimeException("查询考试成绩列表出错", e);
        }
        return records;
    }



    public static ExamRecord queryById(int id) {
        ExamRecord er = new ExamRecord();
        try (Connection connection = DBUtil.getConnection()){
            String sql = "select" +
                    "  er.id," +
                    "  er.score," +
                    "  er.exam_record_desc," +
                    "  er.create_time," +
                    "  e.id            prefix_e_id," +
                    "  e.exam_name     prefix_e_exam_name," +
                    "  e.exam_desc     prefix_e_exam_desc," +
                    "  c.id            prefix_c_id," +
                    "  c.classes_name  prefix_c_classes_name," +
                    "  c2.id           prefix_c2_id," +
                    "  c2.course_name  prefix_c2_course_name," +
                    "  s.id            prefix_s_id," +
                    "  s.create_time   prefix_s_create_time," +
                    "  s.student_name  prefix_s_student_name," +
                    "  s.student_no    prefix_s_student_no," +
                    "  s.id_card       prefix_s_id_card," +
                    "  s.student_email prefix_s_student_email" +
                    " from exam_record er" +
                    "  join exam e on er.exam_id = e.id" +
                    "  join classes c on e.classes_id = c.id" +
                    "  join course c2 on e.course_id = c2.id" +
                    "  join student s on c.id = s.classes_id" +
                    " where er.id=?";
            try(PreparedStatement statement = connection.prepareStatement(sql)){
                statement.setInt(1, id);

                try(ResultSet resultSet = statement.executeQuery()){
                    while(resultSet.next()){
                        //设置考试成绩对象
                        er.setId(resultSet.getInt("id"));
                        er.setScore(resultSet.getBigDecimal("score"));
                        er.setExamRecordDesc(resultSet.getString("exam_record_desc"));
                        er.setCreateTime(new Date(resultSet.getTimestamp("create_time").getTime()));

                        //设置考试对象
                        Exam exam = new Exam();
                        exam.setId(resultSet.getInt("prefix_e_id"));
                        exam.setExamName(resultSet.getString("prefix_e_exam_name"));
                        exam.setExamDesc(resultSet.getString("prefix_e_exam_desc"));
                        er.setExam(exam);

                        //设置班级对象
                        Classes classes = new Classes();
                        classes.setId(resultSet.getInt("prefix_c_id"));
                        classes.setClassesName(resultSet.getString("prefix_c_classes_name"));
                        er.setClasses(classes);

                        //设置课程对象
                        Course course = new Course();
                        course.setId(resultSet.getInt("prefix_c2_id"));
                        course.setCourseName(resultSet.getString("prefix_c2_course_name"));
                        er.setCourse(course);

                        //设置学生对象
                        Student student = new Student();
                        student.setId(resultSet.getInt("prefix_s_id"));
                        student.setCreateTime(new Date(resultSet.getTimestamp("prefix_s_create_time").getTime()));
                        student.setStudentName(resultSet.getString("prefix_s_student_name"));
                        student.setStudentNo(resultSet.getString("prefix_s_student_no"));
                        student.setIdCard(resultSet.getString("prefix_s_id_card"));
                        student.setStudentEmail(resultSet.getString("prefix_s_student_email"));
                        er.setStudent(student);
                    }
                }
            }
        } catch (Exception e) {
            throw new RuntimeException("查询考试详情出错", e);
        }
        return er;
    }

    public static int insert(ExamRecord record) {
        try (Connection connection = DBUtil.getConnection()){
            String sql = "insert exam_record(exam_id,student_id,score) values (?,?,?)";
            try(PreparedStatement statement = connection.prepareStatement(sql)){
                statement.setInt(1, record.getExamId());
                statement.setInt(2, record.getStudentId());
                statement.setBigDecimal(3, record.getScore());
                return statement.executeUpdate();
            }
        } catch (Exception e) {
            throw new RuntimeException("插入考试成绩出错", e);
        }
    }

    public static int update(ExamRecord record) {
        try (Connection connection = DBUtil.getConnection()){
            String sql = "update exam_record set exam_id=?,student_id=?,score=? where id=?";
            try(PreparedStatement statement = connection.prepareStatement(sql)){
                statement.setInt(1, record.getExamId());
                statement.setInt(2, record.getStudentId());
                statement.setBigDecimal(3, record.getScore());
                statement.setInt(4, record.getId());
                return statement.executeUpdate();
            }
        } catch (Exception e) {
            throw new RuntimeException("修改考试成绩出错", e);
        }
    }

    public static int delete(String[] arr) {
        try (Connection connection = DBUtil.getConnection()){
            StringBuilder sql = new StringBuilder("delete from exam_record where (id=");
            for(int i = 0; i < arr.length; i++){
                if(i != 0)
                    sql.append(",");
                sql.append("?");
            }
            sql.append(")");
            try(PreparedStatement statement = connection.prepareStatement(sql.toString())){
                for(int i = 0; i < arr.length; i++){
                    statement.setInt(i + 1, Integer.parseInt(arr[i]));
                }
                return statement.executeUpdate();
            }
        } catch (Exception e) {
            throw new RuntimeException("删除考试成绩出错", e);
        }
    }
}
